...
| 特性 | 描述 | 状态 |
|------|------|------|
| 🗂️ **多源摄入** | 支持 30+ 种格式：Markdown、Word、PPT、PDF、TXT、思维导图、代码、EPUB、图片、网页、邮件、视频、音频，以及通过 markitdown 的更多格式 | ✅ |
| 🔀 **RAG 管道** | 混合搜索(向量+BM25) + 重排序 + 引用生成 + 智能摘要 | ✅ |
...
| 🔌 **插件系统** | 可扩展摄入器注册，4个生命周期钩子，通过 entry_points 插件发现 | ✅ |
| 💾 **导出/导入** | 记忆系统导出/导入，支持多格式备份与分享 | ✅ |
| 📦 **本地化依赖** | 第三方库本地化，模型缓存本地，版本一致性 | ✅ |
